...
## a Unitary, for l layers and n qubits -> unitary network U_{i1,...,in;j1,...jn}
def UniTN_1d_SVD(n: int,l: int,para : List[float], conjugate = False) -> Tuple[List[tn.Node],List[tn.Edge],List[tn.Edge]]:
    ...
    if n%2 == 1:
        ...
        for cnt in range(n//2):
            ...
            uniTen22_22 = UniTen22_22(paralist,conjugate) 
            # SVD decomp
            u_prime, v_prime, _ = tn.split_node(uniTen22_22, left_edges=[uniTen22_22[0],uniTen22_22[2]], right_edges=[uniTen22_22[1],uniTen22_22[3]])
...
def TN2Matrix_SVD(myTN,edge_i,edge_j):
    n = len(edge_i)
    res = np.array([[0.+0.j for i in range(2**n)] for j in range(2**n)])
    s = list(itertools.product(range(2), repeat=n))
    for i in range(2**n):
...
## para: \phi parameters of Unitary tensor network
## T ~ O(n*4^l)
def get_Ham_SVD(n:int,g:float,J:float,l: int, para:list,pauli_str:str) -> np.array:
    contraction_list = [[] for i in range(n)]
    mypara = para.copy()
    if len(mypara) != UniTN_1d_para_cnt(n,l):
        print(mypara)
    ...
    myTN = tn.contractors.auto(tmpNodes,output_edge_order=UniTN[1]+UniTN_conj[1])

    # do contraction and cast into a matrix
    matrix_tens = TN2Matrix_SVD(myTN,UniTN[1],UniTN_conj[1]) 
    return matrix_tens
...
def test1(n,g,J,l):
    ...
    print(np.linalg.eigh(np.array(matrix_tens))[0][0], "time = ",end - start)
    print(spectrum_1dTransverse_Ising(n,g,J)[0])


# test unitary preserves eigenvalues
def test1_SVD(n,g,J,l):
    start = time.time()
    #UniTen22_22([random.uniform(1,10) for i in range(15)])
    #UniTN_1d(6,3,[random.uniform(1,10) for i in range(15*8)])
    #spectrum_1dTransverse_Ising(5,10,0.1)
    n_para = UniTN_1d_para_cnt(n,l) # nums of parameters
    paraList = [random.uniform(1, 10) for i in range(n_para)]    # all parameters
    matrix_tens = get_Ham_SVD(n,g,J,l,paraList)
    end = time.time()
    print(np.linalg.eigh(np.array(matrix_tens))[0][0], "SVD_time = ",end - start)
    print(spectrum_1dTransverse_Ising(n,g,J)[0])
...
def test3():
    paralist = [i*1. for i in range(15)]
    uniTen22_22 = UniTen22_22(paralist)
    print(uniTen22_22.tensor)
    u_prime, v_prime, _ = tn.split_node(uniTen22_22, left_edges=[uniTen22_22[0],uniTen22_22[2]], right_edges=[uniTen22_22[1],uniTen22_22[3]])
    myTN = tn.contractors.auto([u_prime,v_prime],output_edge_order=[u_prime[0],v_prime[1],u_prime[1],v_prime[2]])
    print(myTN.tensor)
