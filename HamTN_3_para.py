...
## update derive and derive_TN
def update_derive(derive_TN):
    #print('hhhhhh',derive_TN)
    if derive_TN == None:
        derive = None
    elif derive_TN >=0 and derive_TN < 3:
        derive = derive_TN
        derive_TN = None
    elif derive_TN != None:
        derive_TN -= 3
    ...
    return derive, derive_TN

##  parameters(a list of 15 real numbers) -> unitary tensor U_{i1,i2;j1,j2}
def UniTen22_22(para: List[float], flag, derive = None, conjugate = False ) -> tn.Node:
    if flag == 1:
        return UniTen22_22_3_para(para, conjugate,derive)
    # debug
    if len(para) != 15:
...
##  parameters(a list of 15 real numbers) -> unitary tensor U_{i1,i2;j1,j2}
def UniTen22_22_3_para(para: List[float], conjugate, derive) -> tn.Node:
    # debug
    if len(para) != 3:
        raise TypeError("len(para) error")
    ## first, we construct a 4*4 unitary matrix by e^{S} where S is a 4*4 anti symmetry
    ...
    H = sum([para[i] * lamb[i] for i in range(3)])
    U44 = 0
    if derive == None:
        U44 = expm(1.j * H)
    elif derive == 0:
        U44 = (1.j*lamb[0]).dot(expm(1.j * H))
    elif derive == 2:
        U44 = expm(1.j * H).dot(1.j*lamb[2])
    elif derive == 1:
        U44 = expm(1.j * para[0] * lamb[0]).dot(1.j*lamb[1]).dot(expm(1.j * para[1] * lamb[1])+1.j * para[2] * lamb[2])
    else:
        raise TypeError('derive_flag not true')
...
def UniTN_1d_para_cnt(n, l, n_one_tensor):
    if n % 2 == 1:
        n_para = n // 2 * l * n_one_tensor  ## number of parameters
    else:
        if l % 2 == 0:
            n_para = (n // 2 + n // 2 - 1) * (l // 2) * n_one_tensor
        else:
            n_para = (n // 2 + n // 2 - 1) * (l // 2) * n_one_tensor + n // 2 * n_one_tensor
    return n_para
...
def UniTN_1d_SVD(n: int, l: int, para: List[float], derive_TN = None, conjugate=False) -> Tuple[
    List[tn.Node], List[tn.Edge], List[tn.Edge]]:
    ## a flag for derivative
    derive = None
    ...
    edge_i = n * [0]
    edge_j = n * [0]
    n_para = UniTN_1d_para_cnt(n, l , 3)

    if len(mypara) != n_para:
        print(n_para)
        raise TypeError("len(para) error")
    elif derive_TN != None and( derive_TN < 0 or derive_TN >= n_para):
        print(derive_TN)
        raise TypeError("derive_TN error")
    ...
    if n % 2 == 1:
        ...
        for cnt in range(n // 2):
            paralist = [mypara.pop(0) for i in range(3)]
            derive,derive_TN = update_derive(derive_TN)
            uniTen22_22 = UniTen22_22(paralist, flag = 1,derive = derive,conjugate = conjugate)
            # SVD decomp
...
## T ~ O(n*4^l)
def get_Ham_Pauli_SVD(n: int, g: float, J: float, l: int, para: list, P_str: str , derive_TN) -> np.array:
    t1 = time.time()
    contraction_list = [[] for i in range(n)]
    mypara = para.copy()
    ...
    # HamTN = Iden(n)
    UniTN, cl_1 = UniTN_1d_SVD(n, l, mypara,derive_TN = derive_TN)
...
## para: \phi parameters of Unitary tensor network
## T ~ O(n*4^l)
def get_Ham_Pauli_SVD_v1(n: int, g: float, J: float, l: int, para: list, derive_TN):
    t1 = time.time()
    contraction_list = [[] for i in range(n)]
    mypara = para.copy()
    if len(mypara) != UniTN_1d_para_cnt(n, l , 3):
        print(mypara)
        raise TypeError("len(para) error")
    # construct U H U^*
    HamTN = Ham2TN_1dTransverse_Ising(n, g, J)
...
def test4():
    ...
    for n in range(n, n + 1):
        ...
        '''
        start = time.time()
        tn_tmp = get_Ham_Pauli_SVD_v1(n, g, J, l, paraList, derive_TN=None)

        for i in range(4**n):
            st = generate_pauli_str(i, n)
            valres = get_Ham_Pauli_SVD_v2(tn_tmp, st, 1,derive_TN=None)
            res = str(valres) + " " + st.replace('', ' ').strip()
            if valres > 1e-12:
                print(res)
        end = time.time()
        time2 = (end - start)
...
def test5():
    ...
    for n in range(n, n + 1):
        ...
        '''
        st = 'XYXYII'
        tn_tmp = get_Ham_Pauli_SVD_v1(n, g, J, l, paraList, derive_TN=0)
        res1 = get_Ham_Pauli_SVD_v2(tn_tmp, st, 1,derive_TN=0)

        print('finite differences')
        h = 0.0001
        tn_tmp = get_Ham_Pauli_SVD_v1(n, g, J, l, paraList, derive_TN=None)
        res2 = get_Ham_Pauli_SVD_v2(tn_tmp, st, 1,derive_TN=None)
        paraList[0] += h
        tn_tmp = get_Ham_Pauli_SVD_v1(n, g, J, l, paraList, derive_TN=None)
        res3 = get_Ham_Pauli_SVD_v2(tn_tmp, st, 1,derive_TN=None)
        res2 = (res3-res2)/h


        print('n = ', n, "res1 = ", res1,"res2 = ", res2, "(new)")
    # get_Ham(n, g, J, l, paraList)  # get Hamiltonian matrix
